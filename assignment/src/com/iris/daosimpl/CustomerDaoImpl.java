...
public class CustomerDaoImpl implements CustomerDao {
	...
	public boolean registerCustomer(Customer cust) throws Exception{
		PreparedStatement ps=conn.prepareStatement("insert into Customer values(?,?,?,?,?,?,'customer')");
  ...
  public Customer validateCustomer(int id, String password) throws Exception{
	...
	if(rs.next())
	{
		...
		Customer cus=new Customer(id,name,gender,email,Password,city,role);
		return cus;
	}
  ...
  public List<Customer> getAllCustomers() throws Exception {
	...
	public boolean updateCustomer(Customer cust) throws Exception {
		PreparedStatement ps=conn.prepareStatement("update customer set name=? gender=? emailaddress=? password=? where id=?");
